...
class BorrowedBooks(metaclass=BorrowedBooksMeta):
    ...
    def delete_record(self, borrowed_record: BorrowedRecord) -> bool:
        """
    ...
    def renew_book(self, borrowed_record: BorrowedRecord, renewal_date:date, new_due_date: date):
        """
        This method renews the book by extending the due date for the book that are already borrowed by the user. It checks for the record
        ...
        :return: None
        """
        if borrowed_record.last_renewal_date is None:
            borrowed_record.last_renewal_date = renewal_date
            renewal_dates = borrowed_record.extras.get('history_of_renewal_dates', [])
            renewal_dates.append(renewal_date)
            borrowed_record.extras.update({'history_of_renewal_dates': renewal_dates})
